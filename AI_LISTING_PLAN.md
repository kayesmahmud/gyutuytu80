...
2. **Phase 2 — AI autofill from photos** (eBay "Magical Listing" / Meta Seller App pattern): the user adds photos first (the form is already photos-first as of commit `7a1d851`); the AI drafts title, description, category + subcategory, condition/brand fields, **and a suggested price** (REVISED 2026-08-26 — pre-filled but fully editable; the earlier "never the price" rule is superseded, the user always has the final say). Every AI-filled field is clearly marked and editable, and a confirmation dialog on "Post Ad" asks the user to verify everything before submitting.
...
### UX contract (agreed with owner — REVISED 2026-08-26, supersedes the earlier "never price" rule)
...
- AI fills: title (short, sellable), description (concise but enough), category + subcategory (validated against the real tree, applied through the existing category-selection code paths), condition ("Brand New"/"Like New"/"Used" judged from the photos), brand/model where confident, **and now PRICE**: the price field comes **pre-filled with DeepSeek's estimate, fully editable**, with the ✨ badge (owner decision 2026-08-26 — eBay/Meta pattern; the ultimate price decision is always the user's). DeepSeek has no Nepali comps data, so the estimate is rough — badge + confirm dialog are mandatory.
...
- If AI is off/unavailable: no badges, no warnings, no suggestions; the form behaves exactly as it does today, keyword-suggestion chip included. The chip remains the instant fallback and should still work while AI is loading. Shop-page location/category prefill still works (it is not AI).
