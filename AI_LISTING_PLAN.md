...
- AI fills: title (short, sellable), description (concise but enough), category + subcategory (validated against the real tree, applied through the existing category-selection code paths), condition ("Brand New"/"Used" judged from the photos, Flutter only), brand/model where confident. **PRICE is NEVER filled** (owner tried the 2026-08-26 pre-fill live and removed it 2026-08-27): the seller types their own price; the server still returns `price_estimate` and clients keep it in state solely for the absurd-price warning below.
