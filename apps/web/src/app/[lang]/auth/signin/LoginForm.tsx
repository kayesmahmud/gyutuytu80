...
export default function LoginForm({ lang }: LoginFormProps) {
  ...
  // Redirect if already logged in, but check for expired tokens first
  useEffect(() => {
    if (status === 'authenticated' && session) {
      // Check if the session token has expired
      // Type assertion needed for extended session properties
      ...
      // If there's an error in the session (like expired token), sign out
      if (sessionWithToken.error === 'RefreshAccessTokenError' ||
        sessionWithToken.error === 'TokenExpired') {
        console.log('Session has expired token, signing out...');
        signOut({ redirect: false });
        return;
      }

      // Check if session expiry is in the past
      if (sessionWithToken.expires) {
        const expiresAt = new Date(sessionWithToken.expires);
