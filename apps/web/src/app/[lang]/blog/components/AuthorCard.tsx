import Link from 'next/link';
import Image from 'next/image';
import { getImageUrl } from '@/lib/images/imageUrl';

interface AuthorCardProps {
  author: {
    name: string;
    name_ne?: string | null;
    slug: string;
    avatar?: string | null;
    bio?: string | null;
    bio_ne?: string | null;
    credentials?: string | null;
    credentials_ne?: string | null;
    social_links?: Record<string, string> | null;
  };
  lang: string;
}

export default function AuthorCard({ author, lang }: AuthorCardProps) {
  const isNe = lang === 'ne';
  const name = (isNe && author.name_ne) || author.name;
  const bio = (isNe && author.bio_ne) || author.bio;
  const credentials = (isNe && author.credentials_ne) || author.credentials;
  const socialLinks = author.social_links as Record<string, string> | null;

  return (
    <div className="bg-gray-50 rounded-xl p-5 sm:p-6 border border-gray-200">
      <div className="flex items-start gap-4">
        {author.avatar && (
          <Image
            src={getImageUrl(author.avatar, 'blog') || '/placeholder-avatar.svg'}
            alt={name}
            width={64}
            height={64}
            className="rounded-full object-cover flex-shrink-0"
          />
        )}
        <div className="min-w-0">
          <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">
            {isNe ? 'लेखक' : 'Written by'}
          </p>
          <Link
            href={`/${lang}/blog/author/${author.slug}`}
            className="text-lg font-bold text-gray-900 hover:text-rose-600 transition-colors"
          >
            {name}
          </Link>
          {credentials && (
            <p className="text-sm text-rose-600 font-medium mt-0.5">{credentials}</p>
          )}
          {bio && (
            <p className="text-sm text-gray-600 mt-2 line-clamp-3">{bio}</p>
          )}
          {socialLinks && (
            <div className="flex gap-3 mt-3">
              {socialLinks.facebook && (
                <a href={socialLinks.facebook} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-600">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
              )}
              {socialLinks.linkedin && (
                <a href={socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-700">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                </a>
              )}
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
