...
import type { ReportedUser, TabStatus } from './types';
...
interface ReportCardProps {
  ...
  onSuspendUser: (reportId: number, reportReason: string) => void;
    ...
    >
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Left: Reported User */}
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Reported User</h4>
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3 mb-3">
                {report.reportedUserAvatar &&
                (report.reportedUserAvatar.startsWith('http') || report.reportedUserAvatar.startsWith('/')) ? (
                  <Image
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900">{report.reportedUserName}</h3>
                  <p className="text-sm text-gray-600">{report.reportedUserEmail}</p>
                </div>
              </div>
              <div className="flex items-center gap-4 text-sm flex-wrap">
                <span className="text-gray-600">User ID: #{report.reportedUserId}</span>
                <span
                  className={`px-2 py-1 rounded text-xs font-medium ${
                    report.reportedUserActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                  }`}
                >
          ...
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-2 uppercase tracking-wide">Report Details</h4>
            <div
            ...
            >
              <div className="mb-3">
                <div className={`text-xs mb-1 font-medium ${activeTab === 'pending' ? 'text-orange-700' : 'text-gray-700'}`}>
                  Reporter:
                </div>
        ...
        <div className="flex gap-3 mt-6 pt-4 border-t border-gray-200 flex-wrap">
          {report.reportedUserShopSlug && (
            <button
              onClick={() => window.open(`/${lang}/shop/${report.reportedUserShopSlug}`, '_blank')}
          ...
          {activeTab === 'pending' && (
            <>
              <button
                onClick={() => onSuspendUser(report.reportId, report.reason)}
                disabled={actionLoading || !report.reportedUserActive}
