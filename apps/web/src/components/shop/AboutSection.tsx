...
interface AboutSectionProps {
  ...
  bio: string | null;
  isOwner: boolean;
  /** When set, a share button renders in the card header (mobile tab view). */
  shareUrl?: string;
}

export function AboutSection({ shopSlug, initialDescription, bio, isOwner, shareUrl }: AboutSectionProps) {
    ...
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
      <div className="flex justify-between items-center mb-3 sm:mb-4">
        ...
        <div className="flex items-center gap-2">
          {shareUrl && <ShareShopButton shopUrl={shareUrl} />}
          {!isEditing && isOwner && (
            <button
