...
export async function initiateEsewaPayment(
  ...
  try {
    ...
    // For mobile apps, return the form URL and data
    // The app should open a WebView and POST to this URL
    const paymentUrl = `${url}?${new URLSearchParams(formData).toString()}`;
...
export async function verifyEsewaPayment(
  ...
  try {
    ...
    return {
      ...
      status,
      transactionId: params.transactionId,
      amount: parseFloat(String(data.total_amount)) || 0,
      gateway: 'esewa',
