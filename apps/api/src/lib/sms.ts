...
function getOtpMessage(otp: string, purpose: OtpPurpose): string {
  switch (purpose) {
    ...
    case 'login':
      return `Your Thulo Bazaar login code is: ${otp}. Valid for 10 minutes. Do not share this code.`;
    case 'password_reset':
    ...
    case 'account_deletion':
      return `Your Thulo Bazaar account deletion code is: ${otp}. Valid for 10 minutes. If you did not request this, ignore this message.`;
    default:
...
const defaultMessages: Record<NotificationType, string> = {
  business_verification_approved: 'Congratulations {name}! Your business verification on Thulo Bazaar has been approved.',
  business_verification_rejected: 'Dear {name}, your business verification on Thulo Bazaar was not approved. Reason: {reason}.',
  business_verification_submitted: 'Dear {name}, your business verification application has been submitted. We will review it within 24-48 hours.',
  ...
  individual_verification_submitted: 'Dear {name}, your identity verification application has been submitted. We will review it within 24-48 hours.',
  account_suspended: 'Dear {name}, your Thulo Bazaar account has been suspended. Reason: {reason}. Contact support for assistance.',
  account_unsuspended: 'Good news {name}! Your Thulo Bazaar account has been restored.',
...
export async function sendOtpSms(
  ...
  try {
    ...
    const data = await response.json() as { error?: boolean; message?: string; response?: string };
