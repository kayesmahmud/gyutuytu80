...
> - ✅ **Post-review hardening (2026-07-11, automated commit review of `051b64e`):** (1) editor-login 2FA now fails **closed** on inconsistent state (`two_factor_enabled` true but `two_factor_secret` null → refuse + route to admin reset, never password-only); (2) `GET /api/editor/profile` — which mounts *before* the router-level ACL gate so login can stay public — now carries `authenticateToken + requireEditorOrAdmin + requireActiveStaff` explicitly; (3) PAY-2 double-spend check moved **inside** the `$transaction` behind a `SELECT … FOR UPDATE` on the payment row (was a TOCTOU window between check and insert — verified race-safe: 5 concurrent requests → 1×201, 4×409, exactly one promotion); (4) initiate now **persists the server-authoritative price** (`priceCheck.expected`) as the stored/charged amount instead of the client's value, so stored amount, gateway charge, and verify-time reconciliation all reference the trusted number (API + web).
